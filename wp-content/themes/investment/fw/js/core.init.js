		...
		if (scroll_offset + jQuery(window).height() > INVESTMENT_STORAGE['side_panel_height'] + 100) {
			if (jQuery('.sidebar_outer').css('position')!=='fixed') {
				jQuery('.sidebar_outer').css({
					//'top': (scroll_offset + jQuery(window).height() - INVESTMENT_STORAGE['side_panel_height'] - 100) + 'px'
					'position': 'fixed',
...
function investment_init_post_formats() {
	...
	} else if (INVESTMENT_STORAGE['popup_engine']=='magnific') {
...
function investment_comments_validate(form) {
	...
	var error = investment_form_validate(form, {
		error_message_text: INVESTMENT_STORAGE['strings']['error_global'],	// Global error message text (if don't write in checked field)
