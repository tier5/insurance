<?php
/**
 * Investment Framework: social networks
 *
 * @package	investment
...
// Theme init
if (!function_exists('investment_socials_theme_setup')) {
	add_action( 'investment_action_before_init_theme', 'investment_socials_theme_setup' );
	function investment_socials_theme_setup() {

		if ( !is_admin() ) {
			// Add og:image meta tag for facebook
			add_action( 'wp_head', 					'investment_facebook_og_tags', 5 );
		}
...
investment_storage_set('share_links', array(
...
//	'facebook' =>		'http://www.facebook.com/sharer.php?s=100&p[url]={link}&p[title]={title}&p[summary]={descr}&p[images][0]={image}',
	...
	'postila' =>		'http://postila.ru/publish/?url={link}&agregator=investment',
...
if (!function_exists('investment_add_share_link')) {
	function investment_add_share_link($soc, $url='') {
		if (!is_array($soc)) $soc = array($soc => $url);
		investment_storage_set('share_links', array_merge( investment_storage_get('share_links'), $soc ) );
	}
}

// Return (and show) share social links
if (!function_exists('investment_show_share_links')) {
	function investment_show_share_links($args) {
		if ( investment_get_custom_option('show_share')=='hide' ) return '';
		...
		$args = array_merge(array(
			...
			'counters' => investment_get_custom_option('show_share_counters')=='yes',	// show share counters
			'direction' => investment_get_custom_option('show_share'),				// share block direction
			'caption' => investment_get_custom_option('share_caption'),				// share block caption
			'share' => investment_get_theme_option('share_buttons'),					// list of allowed socials
		...
		if (is_array($args['share']) && count($args['share']) > 0) {
			foreach ($args['share'] as $soc) {
				...
				$url = $soc['url'];
				if (empty($url)) $url = investment_storage_get_array('share_links', $sn);
				...
				$output .= '<div class="sc_socials_item' . (!empty($args['popup']) ? ' social_item_popup' : '') . '">'
					...
					. ($args['type']=='drop' ? '<i>' . trim(investment_strtoproper($sn)) . '</i>' : '')
					. '</div>';
			}
		}
		$output .= '</div>';
...
if (!function_exists('investment_prepare_socials')) {
	function investment_prepare_socials($list, $style='') {
		if (empty($style)) $style = investment_get_theme_setting('socials_type')=='images' ? 'bg' : 'icons';
		$output = '';
		$upload_info = wp_upload_dir();
		$upload_url = $upload_info['baseurl'];
		if (is_array($list) && count($list) > 0) {
			foreach ($list as $soc) {
				...
				$icon = $style=='icons' || investment_strpos($soc['icon'], $upload_url)!==false ? $soc['icon'] : investment_get_socials_url(basename($soc['icon']));
				if ($style == 'icons') {
					$parts = explode('-', $soc['icon'], 2);
					$sn = isset($parts[1]) ? $parts[1] : $parts[0];
				} else {
...
if (!function_exists('investment_get_twitter_data')) {
	function investment_get_twitter_data($cfg) {
...
if (!function_exists('investment_prepare_twitter_text')) {
	function investment_prepare_twitter_text($tweet) {
		$text = $tweet['text'];
...
if (!function_exists('investment_get_twitter_followers')) {
	function investment_get_twitter_followers($cfg) {
		$data = investment_get_twitter_data($cfg); 
...
if (!function_exists('investment_get_facebook_likes')) {
	function investment_get_facebook_likes($account) {
		$fb = get_transient("facebooklikes");
		if ($fb !== false) return $fb;
		$fb = '?';
		$url = esc_url(investment_get_protocol().'http://graph.facebook.com/'.($account));
		$headers = get_headers($url);
		if (investment_strpos($headers[0], '200')) {
			$json = investment_fgc($url);
...
if (!function_exists('investment_facebook_og_tags')) {
	function investment_facebook_og_tags() {
		global $post;
		if ( !is_singular() || investment_storage_get('blog_streampage')) return;
...
if (!function_exists('investment_get_feedburner_counter')) {
	function investment_get_feedburner_counter($account) {
		$rss = get_transient("feedburnercounter");
		if ($rss !== false) return $rss;
		$rss = '?';
		$url = esc_url(investment_get_protocol().'://feedburner.google.com/api/awareness/1.0/GetFeedData?uri='.($account));
		$headers = get_headers($url);
		if (investment_strpos($headers[0], '200')) {
			$xml = investment_fgc($url);
