...
jQuery(document).ready(function(){
	...
	jQuery('.to_demo_wrap').each(function(idx) {
		"use strict";
		var tab = jQuery(this).parents('.investment_options_tab_content');
		var tab_id = tab.attr('id');
		var scheme = tab_id.substring(4, tab_id.length-8);
		var info = jQuery(this).parents('.investment_options_field_info').attr('id', 'to_demo_scheme_'+scheme).attr('data-scheme', scheme).data('scheme', scheme);
		if (idx==0 && jQuery(this).find('.to_demo_pin').length==1 && investment_get_cookie('to_demo_fixed')==1) investment_options_fix_demo(info);
	});

	// Stick demo panel
	jQuery(document).on('click', '.investment_options_field_info .to_demo_pin', function(e) {
		investment_options_fix_demo(jQuery(this).parents('.investment_options_field_info'));
		e.preventDefault();
		return false;
	});	

	// Update demo on page load
	investment_options_decorate_demo();

	// Update demo on change fields
	jQuery('#partition_schemes_content').on('change', '.investment_options_input,.investment_options_field input[type="hidden"]', function(e) {
		"use strict";
		var tab = jQuery(this).parents('.investment_options_tab_content');
		var tab_id = tab.attr('id');
		var scheme = tab_id.substring(4, tab_id.length-8);
		investment_options_decorate_demo(scheme);
	});

	// Update demo on blur color fields
	jQuery('#partition_schemes_content').on('blur', '.investment_options_input_color', function(e) {
		"use strict";
		var tab = jQuery(this).parents('.investment_options_tab_content');
		var tab_id = tab.attr('id');
		var scheme = tab_id.substring(4, tab_id.length-8);
	...
	jQuery('#partition_schemes_content .investment_options_tab > ul').sortable({
		update: function(event, ui) {
			var tabs = "";
			ui.item.parents('.investment_options_tab').find('> ul > li').each(function() {
				tabs += (tabs ? INVESTMENT_STORAGE['to_delimiter'] : '') + jQuery(this).attr('id').substr(4);
			});
			ui.item.parents('.investment_options_tab').find('> .investment_options_schemes_order').val(tabs);
		}
	}).disableSelection().after('<input type="hidden" name="investment_options_schemes_order" class="investment_options_schemes_order" value="">');
});


// When switch tabs - update fixed demo block
function investment_init_hidden_elements(panel) {
	"use strict";
	var demo = panel.find('.investment_options_field_info[data-scheme]');
	if (demo.length == 0) return;
	var info2 = jQuery('.investment_options').find('>.investment_options_field_info');
...
function investment_options_action_scheme_delete(obj) {
	"use strict";
	var button = jQuery(obj);
	var tab_id = button.parents('.investment_options_tab_content').attr('id');
	var scheme = tab_id.substring(4, tab_id.length-8);
	investment_message_confirm(INVESTMENT_STORAGE['to_strings']['scheme_delete_confirm'], INVESTMENT_STORAGE['to_strings']['scheme_delete']+': "'+scheme+'"', function(btn) {
		"use strict";
		if (btn != 1) return;
		var data = {
			action: 'investment_options_scheme_delete',
			nonce: INVESTMENT_STORAGE['ajax_nonce'],
			scheme: scheme,
			order: button.parents('.investment_options_tab').find('> .investment_options_schemes_order').val()
		};
		setTimeout(function(){
			investment_message_info(INVESTMENT_STORAGE['to_strings']['recompile_styles'], INVESTMENT_STORAGE['to_strings']['wait'], 'spin3 animate-spin', 60000);
		}, 600);
		jQuery.post(INVESTMENT_STORAGE['ajax_url'], data, function(response) {
			...
			if (rez.error === '') {
				investment_message_success(INVESTMENT_STORAGE['to_strings']['scheme_delete_complete']+'<br>'+INVESTMENT_STORAGE['to_strings']['reload_page'], INVESTMENT_STORAGE['to_strings']['scheme_delete']);
				setTimeout(function() { location.reload(); }, 3000);
			} else {
				investment_message_warning(INVESTMENT_STORAGE['to_strings']['scheme_delete_failed']+':<br>'+rez.error, INVESTMENT_STORAGE['to_strings']['scheme_delete']);
			}
		});
...
function investment_options_action_scheme_copy(obj) {
	"use strict";
	var button = jQuery(obj);
	var tab_id = button.parents('.investment_options_tab_content').attr('id');
	var scheme = tab_id.substring(4, tab_id.length-8);
	investment_message_confirm(INVESTMENT_STORAGE['to_strings']['scheme_copy_confirm'], INVESTMENT_STORAGE['to_strings']['scheme_copy']+': "'+scheme+'"', function(btn) {
		"use strict";
		if (btn != 1) return;
		var data = {
			action: 'investment_options_scheme_copy',
			nonce: INVESTMENT_STORAGE['ajax_nonce'],
			scheme: scheme,
			order: button.parents('.investment_options_tab').find('> .investment_options_schemes_order').val()
		};
		...
		jQuery.post(INVESTMENT_STORAGE['ajax_url'], data, function(response) {
			...
			if (rez.error === '') {
				investment_message_success(INVESTMENT_STORAGE['to_strings']['scheme_copy_complete']+'<br>'+INVESTMENT_STORAGE['to_strings']['reload_page'], INVESTMENT_STORAGE['to_strings']['scheme_copy']);
				setTimeout(function() { location.reload(); }, 3000);
			} else {
				investment_message_warning(INVESTMENT_STORAGE['to_strings']['scheme_copy_failed']+':<br>'+rez.error, INVESTMENT_STORAGE['to_strings']['scheme_copy']);
			}
		});
...
function investment_options_fix_demo(info) {
	var info2 = info.parents('.investment_options').toggleClass('to_demo_fixed').find('>.investment_options_field_info');
	if (info2.length == 0) {
		var clonedObj = info.clone();
		clonedObj.removeAttr('id');
		info.parents('.investment_options').append(clonedObj);
		info2 = info.parents('.investment_options').find('>.investment_options_field_info');
	}
	investment_set_cookie('to_demo_fixed', jQuery('.investment_options').hasClass('to_demo_fixed') ? 1 : 0, 365);
}


// Decorate demo block
function investment_options_decorate_demo() {
	"use strict";
	...
	jQuery('.to_demo_body').each(function() {
		...
		var scheme = jQuery(this).parents('.investment_options_field_info').data('scheme');
