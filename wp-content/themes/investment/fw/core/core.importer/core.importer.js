// Investment Importer script

jQuery(document).ready(function(){
	"use strict";
	
	// Show/hide import posts params
	...
	jQuery('.trx_importer_section').on('click', '.trx_buttons input[type="button"]', function() {
		...
		investment_importer_do_action(steps, 0);
	});
});

// Call specified action (step)
function investment_importer_do_action(steps, idx) {
	"use strict";
	...
	// AJAX query params
	var data = {
		ajax_nonce: INVESTMENT_STORAGE['ajax_nonce'],
		action: 'investment_importer_start_import',
	...
	jQuery.post(INVESTMENT_STORAGE['ajax_url'], data, function(response) {
		...
		try {
			...
			rez = { error: INVESTMENT_STORAGE['ajax_error']+':<br>'+response };
		...
		if (rez.error === '') {
			...
			// Do next action
			if (idx < steps.length) {
				investment_importer_do_action(steps, idx);
			} else {
