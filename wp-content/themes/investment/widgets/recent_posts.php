...
// Theme init
if (!function_exists('investment_widget_recent_posts_theme_setup')) {
	add_action( 'investment_action_before_init_theme', 'investment_widget_recent_posts_theme_setup', 1 );
	function investment_widget_recent_posts_theme_setup() {

		// Register shortcodes in the shortcodes list
		//add_action('investment_action_shortcodes_list',	'investment_widget_recent_posts_reg_shortcodes');
		if (function_exists('investment_exists_visual_composer') && investment_exists_visual_composer())
			add_action('investment_action_shortcodes_list_vc','investment_widget_recent_posts_reg_shortcodes_vc');
	}
}

// Load widget
if (!function_exists('investment_widget_recent_posts_load')) {
	add_action( 'widgets_init', 'investment_widget_recent_posts_load' );
	function investment_widget_recent_posts_load() {
		register_widget('investment_widget_recent_posts');
	}
}

// Widget Class
class investment_widget_recent_posts extends WP_Widget {

	function __construct() {
		$widget_ops = array('classname' => 'widget_recent_posts', 'description' => esc_html__('The recent blog posts (extended)', 'investment'));
		parent::__construct( 'investment_widget_recent_posts', esc_html__('Investment - Recent Posts', 'investment'), $widget_ops );
	...
	function form($instance) {
		...
		$counters = array_merge(array('hide' => esc_html__('Hide', 'investment')), investment_get_list_blog_counters(false));
		?>
...
*/
if ( !function_exists( 'investment_sc_widget_recent_posts' ) ) {
	function investment_sc_widget_recent_posts($atts, $content=null){	
		...
		extract($atts);
		$type = 'investment_widget_recent_posts';
		$output = '';
		...
		if ( is_object( $wp_widget_factory ) && isset( $wp_widget_factory->widgets, $wp_widget_factory->widgets[ $type ] ) ) {
			$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '')
						...
						. '">';
			ob_start();
			the_widget( $type, $atts, investment_prepare_widgets_args(investment_storage_get('widgets_args'), $id ? $id.'_widget' : 'widget_recent_posts', 'widget_recent_posts') );
			$output .= ob_get_contents();
			ob_end_clean();
			$output .= '</div>';
		}
		return apply_filters('investment_shortcode_output', $output, 'trx_widget_recent_posts', $atts, $content);
	}
	investment_require_shortcode("trx_widget_recent_posts", "investment_sc_widget_recent_posts");
}


// Add [trx_widget_recent_posts] in the VC shortcodes list
if (!function_exists('investment_widget_recent_posts_reg_shortcodes_vc')) {
	function investment_widget_recent_posts_reg_shortcodes_vc() {
		...
		vc_map( array(
				...
				"name" => esc_html__("Widget Recent Posts", 'investment'),
				"description" => wp_kses_data( __("Insert recent posts list with thumbs and post's meta", 'investment') ),
