...
// Theme init
if (!function_exists('investment_widget_recent_reviews_theme_setup')) {
	add_action( 'investment_action_before_init_theme', 'investment_widget_recent_reviews_theme_setup', 1 );
	function investment_widget_recent_reviews_theme_setup() {

		// Register shortcodes in the shortcodes list
		//add_action('investment_action_shortcodes_list',	'investment_widget_recent_reviews_reg_shortcodes');
		if (function_exists('investment_exists_visual_composer') && investment_exists_visual_composer())
			add_action('investment_action_shortcodes_list_vc','investment_widget_recent_reviews_reg_shortcodes_vc');
	}
}

// Load widget
if (!function_exists('investment_widget_recent_reviews_load')) {
	add_action( 'widgets_init', 'investment_widget_recent_reviews_load' );
	function investment_widget_recent_reviews_load() {
		register_widget('investment_widget_recent_reviews');
	}
}

// Widget Class
class investment_widget_recent_reviews extends WP_Widget {

	function __construct() {
		$widget_ops = array('classname' => 'widget_recent_reviews', 'description' => esc_html__('Recent reviews', 'investment'));
		parent::__construct( 'investment_widget_recent_reviews', esc_html__('Investment - Recent reviews', 'investment'), $widget_ops );
	...
	function widget($args, $instance) {
		...
		if ($q->have_posts()) {
			...
			while ($q->have_posts()) { $q->the_post();
				...
				get_template_part(investment_get_file_slug('templates/_parts/widget-posts.php'));
...
*/
if ( !function_exists( 'investment_sc_widget_recent_reviews' ) ) {
	function investment_sc_widget_recent_reviews($atts, $content=null){	
		...
		extract($atts);
		$type = 'investment_widget_recent_reviews';
		$output = '';
		global $wp_widget_factory;
		if ( is_object( $wp_widget_factory ) && isset( $wp_widget_factory->widgets, $wp_widget_factory->widgets[ $type ] ) ) {
			$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '')
							. ' class="widget_area sc_widget_recent_reviews' 
								. (investment_exists_visual_composer() ? ' vc_widget_recent_posts wpb_content_element' : '') 
								. (!empty($class) ? ' ' . esc_attr($class) : '') 
						. '">';
			ob_start();
			the_widget( $type, $atts, investment_prepare_widgets_args(investment_storage_get('widgets_args'), $id ? $id.'_widget' : 'widget_recent_reviews', 'widget_recent_reviews') );
			$output .= ob_get_contents();
			ob_end_clean();
			$output .= '</div>';
		}
		return apply_filters('investment_shortcode_output', $output, 'trx_widget_recent_reviews', $atts, $content);
	}
	investment_require_shortcode("trx_widget_recent_reviews", "investment_sc_widget_recent_reviews");
}


// Add [trx_widget_recent_reviews] in the VC shortcodes list
if (!function_exists('investment_widget_recent_reviews_reg_shortcodes_vc')) {
	function investment_widget_recent_reviews_reg_shortcodes_vc() {
		...
		vc_map( array(
				...
				"name" => esc_html__("Widget Recent Reviews", 'investment'),
				"description" => wp_kses_data( __("Insert recent reviews list with thumbs and post's meta", 'investment') ),
