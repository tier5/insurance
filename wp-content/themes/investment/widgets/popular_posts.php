...
// Theme init
if (!function_exists('investment_widget_popular_posts_theme_setup')) {
	add_action( 'investment_action_before_init_theme', 'investment_widget_popular_posts_theme_setup', 1 );
	function investment_widget_popular_posts_theme_setup() {

		// Register shortcodes in the shortcodes list
		//add_action('investment_action_shortcodes_list',	'investment_widget_popular_posts_reg_shortcodes');
		if (function_exists('investment_exists_visual_composer') && investment_exists_visual_composer())
			add_action('investment_action_shortcodes_list_vc','investment_widget_popular_posts_reg_shortcodes_vc');
	}
}

// Load widget
if (!function_exists('investment_widget_popular_posts_load')) {
	add_action( 'widgets_init', 'investment_widget_popular_posts_load' );
	function investment_widget_popular_posts_load() {
		register_widget('investment_widget_popular_posts');
	}
}

// Widget Class
class investment_widget_popular_posts extends WP_Widget {

	function __construct() {
		$widget_ops = array('classname' => 'widget_popular_posts', 'description' => esc_html__('The most popular and most commented blog posts (extended)', 'investment'));
		parent::__construct( 'investment_widget_popular_posts', esc_html__('Investment - Most Popular & Commented Posts', 'investment'), $widget_ops );

		// Add thumb sizes into list
		investment_add_thumb_sizes( array( 'layout' => 'widgets', 'w' => 75, 'h' => 75, 'title'=>esc_html__('Widgets', 'investment') ) );
	}
	...
	function widget($args, $instance) {
		...
		$show_counters = isset($instance['show_counters']) && $instance['show_counters'] > 0 ? investment_get_theme_option('blog_counters') : '';
		...
		for ($i=0; $i<3; $i++) {
			...
			if ($i==0) {			// Most popular
				$args['meta_key'] = 'investment_post_views_count';
				$args['orderby'] = 'meta_value_num';
				$show_counters = $show_counters ? 'views' : '';
			} else if ($i==2) {		// Most liked
				$args['meta_key'] = 'investment_post_likes_count';
				$args['orderby'] = 'meta_value_num';
	...
	function form($instance) {
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('title_popular')); ?>"><?php esc_html_e('Most popular tab title:', 'investment'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('title_commented')); ?>"><?php esc_html_e('Most commented tab title:', 'investment'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('title_liked')); ?>"><?php esc_html_e('Most liked tab title:', 'investment'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('number')); ?>"><?php esc_html_e('Number posts to show:', 'investment'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('show_image')); ?>_1"><?php esc_html_e('Show post image:', 'investment'); ?></label><br />
			...
			<label for="<?php echo esc_attr($this->get_field_id('show_image')); ?>_1"><?php esc_html_e('Show', 'investment'); ?></label>
			...
			<label for="<?php echo esc_attr($this->get_field_id('show_image')); ?>_0"><?php esc_html_e('Hide', 'investment'); ?></label>
		</p>

		<p>
			<label for="<?php echo esc_attr($this->get_field_id('show_author')); ?>_1"><?php esc_html_e('Show post author:', 'investment'); ?></label><br />
			...
			<label for="<?php echo esc_attr($this->get_field_id('show_author')); ?>_1"><?php esc_html_e('Show', 'investment'); ?></label>
			...
			<label for="<?php echo esc_attr($this->get_field_id('show_author')); ?>_0"><?php esc_html_e('Hide', 'investment'); ?></label>
		</p>

		<p>
			<label for="<?php echo esc_attr($this->get_field_id('show_date')); ?>_1"><?php esc_html_e('Show post date:', 'investment'); ?></label><br />
			...
			<label for="<?php echo esc_attr($this->get_field_id('show_date')); ?>_1"><?php esc_html_e('Show', 'investment'); ?></label>
			...
			<label for="<?php echo esc_attr($this->get_field_id('show_date')); ?>_0"><?php esc_html_e('Hide', 'investment'); ?></label>
		</p>

		<p>
			<label for="<?php echo esc_attr($this->get_field_id('show_counters')); ?>_1"><?php esc_html_e('Show post counters:', 'investment'); ?></label><br />
...
*/
if ( !function_exists( 'investment_sc_widget_popular_posts' ) ) {
	function investment_sc_widget_popular_posts($atts, $content=null){	
		...
		extract($atts);
		$type = 'investment_widget_popular_posts';
		$output = '';
		global $wp_widget_factory;
		if ( is_object( $wp_widget_factory ) && isset( $wp_widget_factory->widgets, $wp_widget_factory->widgets[ $type ] ) ) {
			$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '')
							. ' class="widget_area sc_widget_popular_posts' 
								. (investment_exists_visual_composer() ? ' vc_widget_popular_posts wpb_content_element' : '') 
								. (!empty($class) ? ' ' . esc_attr($class) : '') 
						. '">';
			ob_start();
			the_widget( $type, $atts, investment_prepare_widgets_args(investment_storage_get('widgets_args'), $id ? $id.'_widget' : 'widget_popular_posts', 'widget_popular_posts') );
			$output .= ob_get_contents();
			ob_end_clean();
			$output .= '</div>';
		}
		return apply_filters('investment_shortcode_output', $output, 'trx_widget_popular_posts', $atts, $content);
	}
	investment_require_shortcode("trx_widget_popular_posts", "investment_sc_widget_popular_posts");
}


// Add [trx_widget_popular_posts] in the VC shortcodes list
if (!function_exists('investment_widget_popular_posts_reg_shortcodes_vc')) {
	//add_action('investment_action_shortcodes_list_vc','investment_widget_popular_posts_reg_shortcodes_vc');
	function investment_widget_popular_posts_reg_shortcodes_vc() {
		...
		vc_map( array(
				...
				"name" => esc_html__("Widget Popular Posts", 'investment'),
				"description" => wp_kses_data( __("Insert popular posts list with thumbs and post's meta", 'investment') ),
				...
				"params" => array(
					...
					array(
						...
						"heading" => esc_html__("Most popular tab title", 'investment'),
						"description" => wp_kses_data( __("Most popular tab title", 'investment') ),
						"class" => "",
						"value" => "",
						"type" => "textfield"
					),
					array(
						...
						"heading" => esc_html__("Most commented tab title", 'investment'),
						"description" => wp_kses_data( __("Most commented tab title", 'investment') ),
						"class" => "",
						"value" => "",
						"type" => "textfield"
					),
					array(
						...
						"heading" => esc_html__("Most liked tab title", 'investment'),
						"description" => wp_kses_data( __("Most liked tab title", 'investment') ),
						"class" => "",
						"value" => "",
						"type" => "textfield"
					),
					array(
						"param_name" => "number",
						"heading" => esc_html__("Number posts to show", 'investment'),
						"description" => wp_kses_data( __("How many posts display in widget?", 'investment') ),
						"admin_label" => true,
						"class" => "",
						"value" => "4",
						"type" => "textfield"
					...
					array(
						...
						"description" => wp_kses_data( __("Select parent category. If empty - show posts from any category", 'investment') ),
						"class" => "",
					...
					array(
						...
						"heading" => esc_html__("Show post's image", 'investment'),
						"description" => wp_kses_data( __("Do you want display post's featured image?", 'investment') ),
					...
					array(
						...
						"heading" => esc_html__("Show post's author", 'investment'),
						"description" => wp_kses_data( __("Do you want display post's author?", 'investment') ),
						"group" => esc_html__('Details', 'investment'),
						"class" => "",
						"std" => 1,
					...
					array(
						...
						"heading" => esc_html__("Show post's date", 'investment'),
						"description" => wp_kses_data( __("Do you want display post's publish date?", 'investment') ),
						"group" => esc_html__('Details', 'investment'),
						"class" => "",
						"std" => 1,
					...
					array(
						...
						"heading" => esc_html__("Show post's counters", 'investment'),
						"description" => wp_kses_data( __("Do you want display post's counters?", 'investment') ),
						"group" => esc_html__('Details', 'investment'),
						"class" => "",
						"std" => 1,
