...
if ( !function_exists( 'investment_template_events_1_theme_setup' ) ) {
	add_action( 'investment_action_before_init_theme', 'investment_template_events_1_theme_setup', 1 );
	function investment_template_events_1_theme_setup() {
		investment_add_template(array(
			'layout' => 'events-1',
			'template' => 'events-1',
			'mode'   => 'events',
			'title'  => esc_html__('Events /Style 1/', 'investment'),
...
// Template output
if ( !function_exists( 'investment_template_events_1_output' ) ) {
	function investment_template_events_1_output($post_options, $post_data) {
		$show_title = true;
							...
							if ($post_data['post_excerpt']) {
								echo in_array($post_data['post_format'], array('quote', 'link', 'chat', 'aside', 'status')) 
										...
										: '<p>'.trim(investment_strshort($post_data['post_excerpt'], isset($post_options['descr']) 
																										? $post_options['descr'] 
																										: investment_get_custom_option('post_excerpt_maxlength_masonry'))).'</p>';
							}
							if (!empty($post_data['post_link']) && !investment_param_is_off($post_options['readmore'])) {
