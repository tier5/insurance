...
if ( !function_exists( 'investment_template_list_theme_setup' ) ) {
	add_action( 'investment_action_before_init_theme', 'investment_template_list_theme_setup', 1 );
	function investment_template_list_theme_setup() {
		investment_add_template(array(
			'layout' => 'list',
			'mode'   => 'blogger',
			'need_columns' => true,
			'title'  => esc_html__('Blogger layout: List', 'investment')
			));
	}
}

// Template output
if ( !function_exists( 'investment_template_list_output' ) ) {
	function investment_template_list_output($post_options, $post_data) {
