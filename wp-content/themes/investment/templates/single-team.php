...
if ( !function_exists( 'investment_template_single_team_theme_setup' ) ) {
	add_action( 'investment_action_before_init_theme', 'investment_template_single_team_theme_setup', 1 );
	function investment_template_single_team_theme_setup() {
		investment_add_template(array(
			'layout' => 'single-team',
			'mode'   => 'team',
			'need_content' => true,
			'need_terms' => true,
			'title'  => esc_html__('Single Team member', 'investment'),
...
// Template output
if ( !function_exists( 'investment_template_single_team_output' ) ) {
	function investment_template_single_team_output($post_options, $post_data) {
		$post_data['post_views']++;
		$show_title = investment_get_custom_option('show_post_title')=='yes';
		...
		if ($show_title && $post_options['location'] == 'center' && investment_get_custom_option('show_page_title')=='no') {
			?>
		...
		if (!$post_data['post_protected'] && (
			...
			(investment_get_custom_option('show_featured_image')=='yes' && $post_data['post_thumb'])	// && $post_data['post_format']!='gallery' && $post_data['post_format']!='image')
		)) {
			?>
		...
		investment_open_wrapper('<section class="post_content'.(!$post_data['post_protected'] && $post_data['post_edit_enable'] ? ' '.esc_attr('post_content_editor_present') : '').'" itemprop="articleBody">');
