...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('investment_sc_tabs_theme_setup')) {
	add_action( 'investment_action_before_init_theme', 'investment_sc_tabs_theme_setup' );
	function investment_sc_tabs_theme_setup() {
		add_action('investment_action_shortcodes_list', 		'investment_sc_tabs_reg_shortcodes');
		if (function_exists('investment_exists_visual_composer') && investment_exists_visual_composer())
			add_action('investment_action_shortcodes_list_vc','investment_sc_tabs_reg_shortcodes_vc');
	}
}
...
if (!function_exists('investment_sc_tabs')) {	
	function investment_sc_tabs($atts, $content = null) {
		...
		investment_storage_set('sc_tab_data', array(
			'counter'=> 0,
            'scroll' => $scroll,
		...
		$sc_tab_titles = investment_storage_get_array('sc_tab_data', 'titles');
		...
		investment_enqueue_script('jquery-effects-fade', false, array('jquery','jquery-effects-core'), null, true);
	
		$tabs_output .= $titles_output
			. '</ul>' 
			. ($content)
			.'</div>';
		return apply_filters('investment_shortcode_output', $tabs_output, 'trx_tabs', $atts, $content);
	}
	investment_require_shortcode("trx_tabs", "investment_sc_tabs");
}


if (!function_exists('investment_sc_tab')) {	
	function investment_sc_tab($atts, $content = null) {
		if (investment_in_shortcode_blogger()) return '';
		extract(investment_html_decode(shortcode_atts(array(
			// Individual params
		...
		investment_storage_inc_array('sc_tab_data', 'counter');
		$counter = investment_storage_get_array('sc_tab_data', 'counter');
		if (empty($id))
			$id = !empty($tab_id) ? $tab_id : investment_storage_get_array('sc_tab_data', 'id').'_'.intval($counter);
		$sc_tab_titles = investment_storage_get_array('sc_tab_data', 'titles');
		...
		investment_storage_set_array('sc_tab_data', 'titles', $sc_tab_titles);
		$output = '<div id="'.esc_attr($id).'"'
					.' class="sc_tabs_content' 
						...
						. ($css!='' ? ' style="'.esc_attr($css).'"' : '') 
						. '>' 
				. (investment_param_is_on(investment_storage_get_array('sc_tab_data', 'scroll')) 
					? '<div id="'.esc_attr($id).'_scroll" class="sc_scroll sc_scroll_vertical" style="height:'.(investment_storage_get_array('sc_tab_data', 'height') != '' ? investment_storage_get_array('sc_tab_data', 'height') : '200px').';"><div class="sc_scroll_wrapper swiper-wrapper"><div class="sc_scroll_slide swiper-slide">' 
					: '')
				...
				. (investment_param_is_on(investment_storage_get_array('sc_tab_data', 'scroll')) 
					...
					: '')
			. '</div>';
		return apply_filters('investment_shortcode_output', $output, 'trx_tab', $atts, $content);
	}
	investment_require_shortcode("trx_tab", "investment_sc_tab");
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'investment_sc_tabs_reg_shortcodes' ) ) {
	//add_action('investment_action_shortcodes_list', 'investment_sc_tabs_reg_shortcodes');
	function investment_sc_tabs_reg_shortcodes() {
	
		investment_sc_map("trx_tabs", array(
			"title" => esc_html__("Tabs", 'investment'),
			"desc" => wp_kses_data( __("Insert tabs in your page (post)", 'investment') ),
			"decorate" => true,
			"container" => false,
			"params" => array(
				"style" => array(
					"title" => esc_html__("Tabs style", 'investment'),
					"desc" => wp_kses_data( __("Select style for tabs items", 'investment') ),
					"value" => 1,
					"options" => investment_get_list_styles(1, 2),
					"type" => "radio"
				),
				"initial" => array(
					"title" => esc_html__("Initially opened tab", 'investment'),
					"desc" => wp_kses_data( __("Number of initially opened tab", 'investment') ),
					"divider" => true,
					"value" => 1,
					"min" => 0,
					"type" => "spinner"
				...
				"scroll" => array(
					...
					"desc" => wp_kses_data( __("Use scroller to show tab content (height parameter required)", 'investment') ),
					"divider" => true,
					"value" => "no",
					"type" => "switch",
			...
			"children" => array(
				"name" => "trx_tab",
				"title" => esc_html__("Tab", 'investment'),
				"desc" => wp_kses_data( __("Tab item", 'investment') ),
				"container" => true,
				"params" => array(
					"title" => array(
						"title" => esc_html__("Tab title", 'investment'),
						"desc" => wp_kses_data( __("Current tab title", 'investment') ),
						"value" => "",
						"type" => "text"
					),
					"_content_" => array(
						"title" => esc_html__("Tab content", 'investment'),
						"desc" => wp_kses_data( __("Current tab content", 'investment') ),
						"divider" => true,
						"rows" => 4,
						"value" => "",
						"type" => "textarea"
...
-------------------------------------------------------------------- */
if ( !function_exists( 'investment_sc_tabs_reg_shortcodes_vc' ) ) {
	//add_action('investment_action_shortcodes_list_vc', 'investment_sc_tabs_reg_shortcodes_vc');
	function investment_sc_tabs_reg_shortcodes_vc() {
	
		$tab_id_1 = 'sc_tab_'.time() . '_1_' . rand( 0, 100 );
		$tab_id_2 = 'sc_tab_'.time() . '_2_' . rand( 0, 100 );
		vc_map( array(
			"base" => "trx_tabs",
			"name" => esc_html__("Tabs", 'investment'),
			"desc" => wp_kses_data( __("Tabs", 'investment') ),
			...
			"params" => array(
				array(
					"param_name" => "style",
					"heading" => esc_html__("Tabs style", 'investment'),
					"desc" => wp_kses_data( __("Select style of tabs items", 'investment') ),
					"admin_label" => true,
					"class" => "",
				...
				array(
					"param_name" => "initial",
					"heading" => esc_html__("Initially opened tab", 'investment'),
					"desc" => wp_kses_data( __("Number of initially opened tab", 'investment') ),
					"class" => "",
					"value" => 1,
					"type" => "textfield"
				...
				array(
					"param_name" => "scroll",
					"heading" => esc_html__("Scroller", 'investment'),
					"desc" => wp_kses_data( __("Use scroller to show tab content (height parameter required)", 'investment') ),
					"class" => "",
			...
			'default_content' => '
				[trx_tab title="' . esc_html__( 'Tab 1', 'investment' ) . '" tab_id="'.esc_attr($tab_id_1).'"][/trx_tab]
				[trx_tab title="' . esc_html__( 'Tab 2', 'investment' ) . '" tab_id="'.esc_attr($tab_id_2).'"][/trx_tab]
			',
			"custom_markup" => '
		...
		vc_map( array(
			"base" => "trx_tab",
			"name" => esc_html__("Tab item", 'investment'),
			"desc" => wp_kses_data( __("Single tab item", 'investment') ),
			...
			"params" => array(
				array(
					"param_name" => "title",
					"heading" => esc_html__("Tab title", 'investment'),
					"desc" => wp_kses_data( __("Title for current tab", 'investment') ),
					"admin_label" => true,
					"class" => "",
					"value" => "",
					"type" => "textfield"
				...
				array(
					"param_name" => "tab_id",
					"heading" => esc_html__("Tab ID", 'investment'),
					"desc" => wp_kses_data( __("ID for current tab (required). Please, start it from letter.", 'investment') ),
					"admin_label" => true,
					"class" => "",
					"value" => "",
					"type" => "textfield"
		...
		class WPBakeryShortCode_Trx_Tabs extends INVESTMENT_VC_ShortCodeTabs {}
		class WPBakeryShortCode_Trx_Tab extends INVESTMENT_VC_ShortCodeTab {}
	}
}
?>