...
function investment_skin_customizer(option, val) {
	...
	if (option == 'body_style') {
		...
		var params = INVESTMENT_STORAGE['co_add_params']!=undefined ? INVESTMENT_STORAGE['co_add_params'] : {};
		params[option] = val;
	...
	if (custom_style != '') {
		var styles = jQuery('#investment-customizer-styles-'+option).length > 0 ? jQuery('#investment-customizer-styles-'+option) : '';
		if (styles.length == 0)
			jQuery('head').append('<style id="investment-customizer-styles-'+option+'" type="text/css">'+custom_style+'</style>');
		else
